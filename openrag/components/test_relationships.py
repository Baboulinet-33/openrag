...
class FileModel(TestBase):
    ...
    def to_dict(self):
        return {
            ...
            "file_metadata": (
                json.loads(self.file_metadata) if self.file_metadata else {}
            ),
    ...
    def get_file_ancestors(self, partition: str, file_id: str) -> list[dict]:
        ...
        with self.Session() as session:
            ...
            result = session.execute(
                query, {"file_id": file_id, "partition": partition}
            )
...
class TestAddFileWithRelationships:
    ...
    def test_add_file_with_relationship_id(self, file_manager):
        ...
        with file_manager.Session() as session:
            result = session.execute(
                text("SELECT relationship_id FROM files WHERE file_id = 'file_001'")
            ).fetchone()
    ...
    def test_add_file_with_parent_id(self, file_manager):
        ...
        with file_manager.Session() as session:
            result = session.execute(
                text("SELECT parent_id FROM files WHERE file_id = 'file_001'")
            ).fetchone()
    ...
    def test_add_file_with_both_relationship_and_parent(self, file_manager):
        ...
        with file_manager.Session() as session:
            result = session.execute(
                text(
                    "SELECT relationship_id, parent_id FROM files WHERE file_id = 'file_002'"
                )
            ).fetchone()
            assert result[0] == "thread_abc123"
    ...
    def test_add_file_without_relationships(self, file_manager):
        ...
        with file_manager.Session() as session:
            result = session.execute(
                text(
                    "SELECT relationship_id, parent_id FROM files WHERE file_id = 'file_003'"
                )
            ).fetchone()
