    ...
    for doc in results:
        ...
        if include_related and metadata.get("relationship_id"):
            relationship_ids.add(
                (metadata.get("partition"), metadata.get("relationship_id"))
            )
...
async def search_multiple_partitions(
    ...
    include_related: bool = Query(
        False, description="Include chunks from files with same relationship_id"
    ),
    include_ancestors: bool = Query(
        False, description="Include chunks from ancestor files in hierarchy"
    ),
